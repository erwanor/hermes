...
impl ChainEndpoint for PenumbraChain {
    ...
    fn version_specs(&self) -> Result<crate::chain::cosmos::version::Specs, Error> {
        let raw_uri = format!("{}", self.config.grpc_addr);
        let url = Url::from_str(&raw_uri.as_str())
            .map_err(|e| Error::temp_penumbra_error(e.to_string()))?;

        let client = tendermint_rpc::HttpClient::new(url.clone())
            .map_err(|e| Error::temp_penumbra_error(e.to_string()))?;

        let version_specs = self
            .rt
            .block_on(fetch_version_specs(self.id(), &client, &url))?;
        Ok(version_specs)
    }
